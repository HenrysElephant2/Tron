...
unsigned int LoadTransBMP(const char* file)
{
   ...
   FILE*          f;          // File pointer
   unsigned short magic;      // Image magic
   unsigned int   dx,dy,size,asize; // Image dimensions
   unsigned short nbp,bpp;    // Planes and bits per pixel
   unsigned char* image, *aimage;      // Image data
   unsigned int   off;        // Image offset
   unsigned int   k;          // Counter
   int            max;        // Maximum texture dimensions
   ...
   asize = 4*dx*dy;
   aimage = (unsigned char*) malloc(asize);
   for(k=0; k<asize; k+=4) {
      aimage[k] = image[k*3/4];
   ...
   glBindTexture(GL_TEXTURE_2D,texture);
   //  Copy image
   glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,dx,dy,0,GL_RGBA,GL_UNSIGNED_BYTE,aimage);
   if (glGetError()) Fatal("Error in glTexImage2D %s %dx%d\n",file,dx,dy);
   //  Scale linearly when image size doesn't match
   glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
   glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
...
void ErrCheck(const char* where) // causing a warning with gluErrorString, so that is disabled
{
   int err = glGetError();
